...
public class CentralizedSimulationLauncher {
	...
	public static void run(List<String> linksToDisable) throws AnnotationProcessorException, KeyStoreException {		
		...
		for (i = 0; i < Settings.VEHICLE_COUNT; i++) {
			createAndDeployVehicleComponent(i, distributor.getLinkId(), (destinations.remove(r.nextInt(destinations.size())) == 0) ? leftDestination : rightDestination);
		}
		
		processor.process(CapacityExchangeEnsemble.class, VehicleDestinationsExachangeEnsemble.class);
		
		DirectSimulationHost host = simulation.getHost("Host");
		RuntimeFramework runtime = builder.build(host, simulation, null, model, new AlwaysRebroadcastingKnowledgeDataManager(model.getEnsembleDefinitions(), null), new CloningKnowledgeManagerFactory(), SecurityKeyManagerImpl.getInstance(), RatingsManagerImpl.getInstance());
		runtime.start();
		...
		System.out.println("Max parking time: " + MaxParkingTimeCollector.getInstance().getMaxTime());
	}
